...
\begin{itemize}
    \item Total lines collected: 776,810
    \item Estimated tokens: 135,361,900 (135.4M)
    ...
    \item Target: 500M tokens | Progress: 27.1\%
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Total}: 776.8K lines training data (27.1\% toward 500M token target)
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{plot_data_collection.png}
    \caption{
        \textbf{Left}: Total lines per language. Telugu has 32.5x more training data than Bhojpuri.
        \textbf{Right}: Estimated tokens. Telugu already exceeds 500M target; Bhojpuri at 27.1\% of target.
    }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{plot_cleaning_results.png}
    \caption{
...
\subsection{Data Source Composition (Telugu vs Bhojpuri)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plot_data_sources_both.png}
    \caption{
        \textbf{Left}: Telugu sources dominated by manual corpus (84.6\%), with OCR augmentation (14.9\%) and web scraping (0.1\%).
        \textbf{Right}: Bhojpuri sources more diverse - HuggingFace corpus primary, substantial OCR augmentation,
        plus web scraping and machine translation to boost lower-resource data.
    }
    \label{fig:sources_both}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{plot_token_distribution.png}
    \caption{
        Token distribution: Telugu (4.4B tokens, 884.4\% of goal) vs Bhojpuri (135.4M tokens, 27.1\% of goal).
    }
    \label{fig:token_progress}
\end{figure}

\subsection{Phase 1 Comprehensive Summary}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{plot_phase1_summary.png}
    \caption{
        Comprehensive overview showing data collection breakdown by source (top), key statistics for both languages (bottom left),
        and tokenizer evaluation summary (bottom right).
    }
...
\begin{table}[H]
    ...
    \textbf{Split} & \textbf{Lines} & \textbf{Percentage} \\
    \hline
    \textbf{Train} & 621,769 & 80.0\% \\
    Validation & 77,358 & 10.0\% \\
    Test & 77,683 & 10.0\% \\
    \hline
    \textbf{Total} & 776,810 & 100.0\% \\
    \hline
    \end{tabular}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{plot_tokenizer_analysis.png}
    \caption{
        Comprehensive tokenizer evaluation across all 6 variants.
        \textbf{Top-Left}: Vocabulary size vs actual tokens used (usage efficiency).
        \textbf{Top-Right}: Average characters per token (fertility) showing byte-level compression vs linguistic units.
        \textbf{Bottom-Left}: Unknown token rates demonstrating near-perfect corpus coverage (0.0000-0.0001\%).
        \textbf{Bottom-Right}: Summary statistics and key metrics.
    }
...
\begin{itemize}
    ...
    \item \textbf{Unique Tokens Used}: 15,643
    \item \textbf{Average Characters per Token}: 3.4369
    \item \textbf{Coverage}: 97.77\%
    \item \textbf{Unknown Token Rate}: 0.0\%
    \item \textbf{Top Tokens}: ``-'' (0.40\%), {\devanagariFont ।} (0.35\%), {\devanagariFont के} (0.30\%)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Vocabulary Size}: 16,000
    \item \textbf{Test Lines Evaluated}: 77,683
    \item \textbf{Test Tokens Evaluated}: 5,930,926
    \item \textbf{Unknown Token Rate}: 0.0001\%
    \item \textbf{Top Tokens}: {\devanagariFont के} (191,018), {\devanagariFont ।} (151,048), comma (119,912)
\end{itemize}
