...
class TeluguDataCleaner:
    def __init__(self, input_dir: str = "./data/raw", output_dir: str = "./data/cleaned"):
        ...
        self.telugu_start = 0x0C00
        self.telugu_end = 0x0C7F

        # Approved characters for SLM training
    ...
    def _is_telugu_char(self, char: str) -> bool:
        """Check if character is Telugu Unicode"""
        return self.telugu_start <= ord(char) <= self.telugu_end

    def _compute_telugu_ratio(self, text: str) -> float:
        """Compute ratio of Telugu characters in text"""
        if not text:
            return 0.0
        telugu_count = sum(1 for c in text if self._is_telugu_char(c))
        return telugu_count / len(text)
    ...
    def _strip_zero_width_chars(self, text: str) -> str:
        ...
        # Remove zero-width joiner, non-joiner, space, etc.
        text = re.sub(r'[​‌‍‎‏﻿]', '', text)
    ...
    def _whitelist_characters(self, text: str) -> str:
        """Keep only Telugu + digits + approved punctuation + whitespace"""
        result = []
        for char in text:
            if self._is_telugu_char(char) or char.isdigit() or char in self.approved_punct:
                result.append(char)
        return ''.join(result)
    ...
    def clean_text(self, text: str) -> str:
        ...
        # 4. Whitelist characters (Telugu + digits + approved punctuation)
    ...
    def is_valid_for_training(self, text: str) -> tuple[bool, str]:
        ...
        Reason is None if valid, otherwise a string describing why it was rejected.
        """
        if not text or len(text.strip()) == 0:
            return False, 'empty'

        # Pre-strip Telugu ratio check (before whitelisting)
        # Check on original text to catch English-dominant lines
        pre_strip_ratio = self._compute_telugu_ratio(text)
        ...
        # Space density check (catches glued TOC like "1.4ఇతర1.5గద్య")
        space_density = self._compute_space_density(text)
        if space_density < 0.05:  # Less than 5% spaces is suspicious
    ...
    def deduplicate_texts(self, texts: List[str]) -> tuple[List[str], int]:
        ...
        Returns (unique_texts, num_duplicates_removed).
        """
        exact_seen = set()
        normalized_seen = set()
        unique = []
        ...
        for text in texts:
            ...
            # Near-match check: normalize by stripping whitespace/punctuation
