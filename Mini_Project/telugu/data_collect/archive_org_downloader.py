...
class TeluguArchiveOrgDownloader:
    ...
    SEARCH_STRATEGIES = [
        ...
        "subject:(Telugu) AND mediatype:(texts)",
        "creator:(Telugu) OR language:(Telugu)",
        "mediatype:(texts) AND (title:(telugu) OR description:(telugu))",
    ]

    def __init__(self, language_query="Telugu", script_range=(0x0C00, 0x0C7F), output_dir=None,
    ...
    def run(self, target_new_items=None, max_runtime_seconds=None):
        ...
        batch = []
        logger.info("=" * 70)
        logger.info(f"Archive.org Downloader: {self.language_query}")
        logger.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Download books from archive.org")
    parser.add_argument("--target-new-items", type=int, default=None)
    parser.add_argument("--state-file", type=Path, default=None)
    parser.add_argument("--output-dir", type=Path, default=None)
    args = parser.parse_args()
    downloader = TeluguArchiveOrgDownloader(state_file=args.state_file, output_dir=args.output_dir)
