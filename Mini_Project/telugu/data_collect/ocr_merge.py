#!/usr/bin/env python3
"""
Merge OCR-cleaned texts into split datasets and accumulator file.

After ocr_extractor.py + existing TeluguDataCleaner produce ocr_cleaned/*.jsonl,
this script:
1. Reconstructs telugu.txt from existing split fragments if absent
...
4. Appends the three new chunks onto existing train/val/test/telugu.txt files
5. Appends the full new batch onto the accumulator telugu.txt
...
def merge_ocr_into_splits(
    data_dir: Path,
    accumulator_filename: str = "telugu.txt",
    split_filename: str = "telugu.txt",
) -> Dict:
    """
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Merge OCR data into splits")
    parser.add_argument(
        '--data-dir',
        type=Path,
