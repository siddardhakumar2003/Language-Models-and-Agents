#!/usr/bin/env python3
"""
OCR-based text extraction from scanned books/news images.

Walks a source directory of PDFs and images, extracts text via Tesseract,
applies OCR-specific pre-cleaning, and outputs JSONL batches compatible
with the existing TeluguDataCleaner.
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Extract text from scanned documents via OCR")
    parser.add_argument(
        '--source-dir',
        help='Directory containing PDFs/images (default: telugu/data_collect/ocr_sources/)'
    )
    parser.add_argument(
        '--output-dir',
        help='Output directory for JSONL batches (default: telugu/data/ocr_raw/)'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
    ...
    extractor = TeluguOCRExtractor(source_dir=args.source_dir, output_dir=args.output_dir)
