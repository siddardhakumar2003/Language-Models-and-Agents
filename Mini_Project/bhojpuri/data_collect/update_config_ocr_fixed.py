...
def update_config_bhojpuri(config_path, data_dir):
    ...
    total_tokens, total_bytes = compute_corpus_tokens_estimate(data_dir, language="bhojpuri")
    logger.info("Reading OCR cumulative stats...")
    cumulative_stats = get_cumulative_ocr_stats(data_dir)
    logger.info("Recomputing splits...")
    splits = recompute_splits_block(data_dir, language="bhojpuri")
    config["ocr_data"] = {"source": "archive.org pre-OCR'd text (Hindi proxy) + news crawling",
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Update Bhojpuri config.json (fixed)")
    parser.add_argument("--data-dir", type=Path, default=None)
    args = parser.parse_args()
    if args.data_dir is None:
        args.data_dir = Path(__file__).resolve().parent.parent / "data"
    update_config_bhojpuri(str(args.data_dir / "config.json"), str(args.data_dir))
