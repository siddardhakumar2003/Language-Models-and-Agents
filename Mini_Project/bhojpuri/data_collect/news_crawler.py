...
class BhojpuriNewsCrawler:
    DOMAINS = [
        {"name": "bbc_hindi", "base_url": "https://www.bbc.com/hindi"},
        {"name": "aajtak", "base_url": "https://www.aajtak.in"},
        {"name": "amarujala", "base_url": "https://www.amarujala.com"},
        {"name": "bhaskar", "base_url": "https://www.bhaskar.com"},
        {"name": "jagran", "base_url": "https://www.jagran.com"},
        {"name": "ndtv_hindi", "base_url": "https://khabar.ndtv.com"},
        {"name": "zee_hindi", "base_url": "https://zeenews.india.com/hindi"},
        {"name": "aaj_news", "base_url": "https://www.aaj.tv"},
        {"name": "webdunia", "base_url": "https://hindi.webdunia.com"},
        {"name": "thehindu_hindi", "base_url": "https://www.thehindu.com"},
    ]
    ...
    def _is_devanagari_text(self, text):
        if not text or len(text) < 20:
            return False
        count = sum(1 for c in text if 0x0900 <= ord(c) <= 0x097F)
        return (count / len(text)) >= 0.30
    ...
    def run(self, target_new_articles=None, max_runtime_seconds=None):
        ...
        logger.info("=" * 70)
        logger.info("News Crawler: Hindi (Bhojpuri proxy)")
        logger.info("=" * 70)
        ...
        for domain_cfg in self.DOMAINS:
            ...
            for url in urls[:100]:
                ...
                if not text or not self._is_devanagari_text(text):
                    articles_failed += 1
                    continue
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Crawl news sites for Hindi text (Bhojpuri proxy)")
    parser.add_argument("--target-new-articles", type=int, default=None)
    parser.add_argument("--output-dir", type=Path, default=None)
    args = parser.parse_args()
    crawler = BhojpuriNewsCrawler(output_dir=args.output_dir)
