#!/usr/bin/env python3
"""
Merge multiple English→Bhojpuri translation batches from different folders.
Efficiently processes multiple folders in one pipeline run.
"""
import json
import logging
import shutil
...
def reshape_translations_from_folders(data_dir: Path, source_folders: list, raw_dir: Path) -> int:
    """
    Stream translations.jsonl from multiple folders, extract 'bho' field only,
    write reshaped single JSONL file into raw_dir in {"text": ...} format.

    Args:
        data_dir: Path to bhojpuri/data
        source_folders: List of folder names (e.g., ["Translated/bhojpuri_translations", "Translated/translations_2", ...])
        raw_dir: Output raw directory
    ...
    logger.info(f"Reshaping {len(source_folders)} translation folders to {output_file.name}")

    with open(output_file, 'w', encoding='utf-8') as fout:
        for folder_name in source_folders:
            source_jsonl = data_dir / folder_name / "translations.jsonl"

            if not source_jsonl.exists():
                logger.warning(f"Skipped (not found): {source_jsonl}")
                continue

            logger.info(f"Processing: {source_jsonl}")
            ...
            with open(source_jsonl, 'r', encoding='utf-8') as fin:
                for line_num, line in enumerate(fin, 1):
                    try:
                        ...
                        if line_num % 10000 == 0:
                            logger.warning(f"Skipped unparseable JSON at line {line_num} in {folder_name}")
                        continue
    ...
    logger.info(
        f"Reshaped {processed_folders} folders to {output_file.name}: "
...
def dedupe_against_corpus(data_dir: Path) -> None:
    """Deduplicate cleaned batch against existing corpus."""
...
def patch_config(data_dir: Path, raw_count: int, cleaned_kept: int, merge_result: dict, folder_names: list) -> None:
    """Patch config.json with updated stats."""
    ...
    # Update phase3_fineweb_translation block with multiple folders
    folders_str = ", ".join(folder_names)
    config["phase3_fineweb_translation"] = {
        "source": f"fineweb-edu dataset (Kaggle: nameonlu/fineweb-edu, folders: {folders_str})",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Merge multiple English→Bhojpuri fineweb-edu translation batches into corpus"
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=["Translated/2/bhojpuri_translations", "Translated/3/bhojpuri_translations"],
        help="Translation folder names to process (default: folders 2 and 3)"
    )
    ...
    logger.info("=" * 70)
    logger.info("MULTIPLE ENGLISH → BHOJPURI TRANSLATION MERGE")
    logger.info("=" * 70)
    logger.info(f"Data dir: {data_dir}")
    logger.info(f"Folders to process: {', '.join(args.folders)}")

    # Step 1: Reshape
    raw_count = reshape_translations_from_folders(data_dir, args.folders, data_dir / RAW_DIR_NAME)
    if raw_count == 0:
    ...
    # Step 7: Patch config
    patch_config(data_dir, raw_count, cleaned_kept, merge_result, args.folders)

    logger.info("=" * 70)
    logger.info("MERGE COMPLETE")
    logger.info("=" * 70)
    logger.info(f"  Folders processed: {len(args.folders)}")
