#!/usr/bin/env python3
"""
Hindi to Bhojpuri translation via unofficial Google Translate endpoint.
Resumable, with circuit breaker and state tracking.
"""
import json
import logging
import random
...
def translate_chunk(session, text, sl="hi", tl="bho", timeout=15):
    """Translate a single chunk via Google Translate (unofficial endpoint)."""
    try:
        params = {
            'client': 'gtx',
            'sl': sl,
...
def translate_long_text(text, delay_seconds=1.5, jitter_seconds=0.5, max_retries=3):
    """Translate long text with chunking, retry logic, and rate limiting."""
    ...
    for i, chunk in enumerate(chunks):
        ...
        translated = translate_chunk(session, chunk)
        if translated is None:
            logger.warning(f"Failed to translate chunk {i}/{len(chunks)}, returning None")
            return None
...
class HindiToBhojpuriTranslator:
    """Resumable Hindi→Bhojpuri translator with state tracking and circuit breaker."""
    ...
    def self_test(self):
        ...
        logger.info("Running self-test with 5 Hindi samples...")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Hindi to Bhojpuri Translator")
